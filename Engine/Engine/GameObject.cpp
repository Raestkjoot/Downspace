...
namespace MyEngine {
	...
	void GameObject::Render(sre::SpriteBatch::SpriteBatchBuilder& builder) {
		if (!_isActive) return;
	...
	void GameObject::SetPosition(glm::vec3 position) {
		...
		// If we want child objects to move with parent:
		//auto posDiff = GetPosition() - prevPos;
	...
	void GameObject::SetActive(bool isActive) {
		...
		// set active on physics bodies
		auto body = FindComponent<ComponentPhysicsBody>();
		if (!body.expired()) {
			body.lock()->SetActive(isActive);
		}
		// set active on sprite renderers
		auto renderer = FindComponent<ComponentRendererSprite>();
		if (!renderer.expired()) {
			renderer.lock()->SetActive(isActive);
		}

		// set active on child gameobjects
		for (auto& child : _children)
			if (auto go = child.lock())
				go->SetActive(isActive);

		// On Enabled/Disabled callback in components
		if (isActive) {
			for (auto& component : _components)
				component->OnEnabled();
		}
