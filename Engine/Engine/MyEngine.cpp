...
namespace MyEngine {
	...
	void Engine::Render() {
		sre::RenderPass renderPass = sre::RenderPass::create()
			.withCamera(_camera)
			.withClearColor(true, _backgroundColor)
			.build();
	...
	void Engine::SetBackgroundColor(sre::Color color) {
		_backgroundColor = color;
	}

	float Engine::RandomRange(float min, float max) {
		return min + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (max - min)));
	}

	float Engine::RandomRange(glm::vec2 range) {
