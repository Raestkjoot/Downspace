...
void ComponentPhysicsBody::Init(rapidjson::Value& serializedData) {
	...
	ShapeType shapeType = static_cast<ShapeType>(serializedData["shapeType"].GetInt());

	CreateBody(static_cast<b2BodyType>(bodyType), isSensor, size, shapeType);
}

void ComponentPhysicsBody::CreateBody(b2BodyType bodyType, bool isSensor, glm::vec2 size, ShapeType shapeType) {
	...
	case Circle:
		...
		dynamic_cast<b2CircleShape*>(_shape)->m_radius = std::max(size.x, size.y) / physicsScale;
		break;
	default:
		MyEngine::Logger::Log("Invalid shape when trying to create physics body! Creating a box instead.");
	case Box:
		_shape = new b2PolygonShape();
		dynamic_cast<b2PolygonShape*>(_shape)->SetAsBox(size.x / physicsScale, size.y / physicsScale);
		break;
	}
...
void ComponentPhysicsBody::SetGravityScale(float scale) {
	_body->SetGravityScale(scale);
}

void ComponentPhysicsBody::SetActive(bool isActive) {
	_body->SetActive(isActive);
}

void ComponentPhysicsBody::SetStaticBodyPosition(glm::vec2 pos) {
	//100.f = MyEngine::Engine::GetInstance()->PHYSICS_SCALE;
	b2Vec2 bodyPos = { pos.x / 100.f, pos.y / 100.f };
	_body->SetTransform(bodyPos, _body->GetAngle());
}